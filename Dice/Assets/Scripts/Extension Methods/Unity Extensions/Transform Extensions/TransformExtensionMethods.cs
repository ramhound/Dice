...
public static partial class TransformExtensions {
    ...
    /// <param name="tag"></param>
    /// <returns></returns>
    public static T FindByTypeRecursively<T>(this Transform transform) where T : Component {
        for(int i = 0; i < transform.childCount; i++) {
            T type = transform.GetChild(i).gameObject.GetComponent<T>();
            if(type != null)
                return type;
        ...
        for(int i = 0; i < transform.childCount; i++) {
            T returnedType = transform.GetChild(i).FindByTypeRecursively<T>();
    ...
    public static T FindChildByTypeRecursively<T>(this Transform transform) where T : Component {
        return transform.FindByTypeRecursively<T>();
    }
